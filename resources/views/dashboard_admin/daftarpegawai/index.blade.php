...
<div class="bg-blue-100 dark:bg-slate-500 p-6 grid grid-cols-1 lg:grid-cols-2 mt-5 rounded-sm">
    ...
    <div class="col-span-3 flex justify-end mb-5">
        <label for="tambah_unit_kerja" class="btn bg-blue-300 hover:bg-blue-400 border-white hover:border-white dark:bg-blue-800 text-black-2 dark:text-white">
            Tambah Unit Kerja
        </label>
    </div>
            ...
            <tbody>
                @foreach ($unit_kerja as $index => $item)
                    <tr class="border-b hover:bg-gray-100">
                        ...
                        <td class="p-2">
                            <label for="edit_unit_kerja_{{ $item->id }}" class="bg-yellow-400 rounded p-2 mr-2 cursor-pointer">
                                    ...
                                    <form action="{{ route('admin.update_unit_kerja', $item->id) }}" method="POST">
                                        ...
                                        <div class="grid grid-cols-1 gap-1">
                                            <div class="form-control">
                                                <label for="nama_unit" class="text-black-2 mt-5 mb-2 text-sm lg:text-lg dark:text-white">Unit Kerja</label>
                                                <input type="text" class="input input-bordered bg-white dark:text-black-2 w-full" name="nama_unit" id="nama_unit" value="{{ $item->nama_unit }}" required>
                                            </div>
                                            <div class="form-control col-span-2 flex items-center justify-center mt-5">
                                                <button type="submit" class="btn w-full lg:w-auto bg-blue-400 hover:bg-blue-300 dark:bg-black text-white">Edit</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <label for="delete_unit_kerja_{{ $item->id }}" class="bg-red-500 rounded p-2 cursor-pointer">
                            ...
                            <div class="modal" role="dialog">
                                <div class="modal-box dark:bg-gradient-to-b bg-blue-300 dark:from-blue-900 dark:to-blue-950">
                                    <form action="{{ route('admin.delete_unit_kerja', $item->id) }}" method="POST">
                                        ...
                                        <p class="text-sm mt-5 text-black-2 dark:text-white">Anda Yakin Mau Menghapus unit kerja {{ $item->nama_unit }}?</p>
                                        <p class="text-sm mt-5 text-black-2 dark:text-white">Tindakan bersifat Irreversible</p>
                                        <div class="form-control col-span-2 flex items-center justify-center mt-5">
                                            <button type="submit" class="btn w-full lg:w-auto bg-red text-white">Delete</button>
                                        </div>
                                    </form>
                                </div>
...
<div class="bg-blue-100 dark:bg-slate-500 p-6 grid grid-cols-1 lg:grid-cols-2 mt-5 rounded-sm">
    ...
    <div class="col-span-3 flex justify-end mb-5">
        <a href="{{ route('add_user') }}" class="btn bg-blue-300 hover:bg-blue-400 border-white hover:border-white dark:bg-blue-800 text-black-2 dark:text-white">Tambah User</a>
    </div>
    <div class="col-span-3">
                ...
                @foreach ($users as $index => $item)
                    <tr class="border-b hover:bg-gray-100">
                        ...
                        <td class="p-2 text-left">
                            <a href="{{ route('show_user_info', $item->id) }}" class="bg-yellow-400 rounded p-1 px-2 py-2">
                            ...
                            <div class="modal" role="dialog">
                                <div class="modal-box dark:bg-gradient-to-b bg-blue-300 dark:from-blue-900 dark:to-blue-950">
                                    <form action="{{ route('delete_user', $item->id) }}" method="POST">
                                        ...
                                        @method('DELETE')
                                        <h3 class="text-lg text-black-2 dark:text-white font-bold">Peringatan!</h3>
                                        <p class="text-sm mt-5 text-black-2 dark:text-white">Anda Yakin Mau Menghapus user {{ $item->nama }}?</p>
                                        <p class="text-sm mt-5 text-black-2 dark:text-white">Tindakan bersifat Irreversible</p>
                                        <div class="form-control col-span-2 flex items-center justify-center mt-5">
                                            <button type="submit" class="btn w-full lg:w-auto bg-red text-white">Delete</button>
                                        </div>
