                    ...
                    <tr class="border-b bg-gray-100 text-black-2 hover:bg-gray-100">
                        ...
                        <td class="p-3 border-b ">
                            ...
                            <form action="{{ route('delete.user.role', ['user' => $user->id, 'set_role' => $setRole->id]) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Anda Yakin Mau Menghapus Role Dari User {{ $user->nama }} Dari Role {{  $setRole->role->nama_role }} ? ')" class="btn bg-red-500 text-white py-1 px-4 rounded-lg hover:bg-red-600 transition ml-2">                                    <i class="fa-solid fa-trash text-white"></i>
                                </button>
                            </form>
                        </td>
                    </tr>
    ...
    <div class="bg-blue-100 dark:bg-slate-400 p-6 rounded-md mt-5 mb-5 text-black">
        ...
        <form action="{{ route('add.user.role', $user->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="id_role" id="role" class="block w-full mt-1 p-2 border-gray-300 bg-white rounded-md select select-bordered">
            ...
            <div class="mb-4">
                ...
                <select name="id_unit_kerja" id="unit_kerja" class="block w-full mt-1 p-2 border-gray-300 rounded-md select select-bordered bg-white">
            ...
            <div class="flex justify-end">
                <button type="submit" class="btn bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded-lg">Tambah Role</button>
            </div>
        </form>
    </div>
</div>
