...
<div class="mb-3">
    <h1 class="text-2xl font-semibold">
        ...
        <span class="dark:text-white text-blue-500">Edit User</span>
    </h1>
</div>
...
<div class="bg-blue-100 dark:bg-slate-500 p-6 rounded-md mt-5">
    ...
    <form action="{{ route('edit_user_info', $data->id) }}" method="POST">
        @csrf
        @method("PATCH")
        <div class="grid grid-cols-2 gap-4">
            <div class="form-control">
                <label for="NPM" class="text-black-2 dark:text-white mb-2">NPM/NIS/NIK</label>
                <input type="text" name="NPM" id="NPM" class="input input-bordered text-black-2 bg-white w-full" value="{{ $data->NPM }}" required>
            </div>
            <div class="form-control">
                <label for="nama" class="text-black-2 dark:text-white mb-2">Nama</label>
                <input type="text" name="nama" id="nama" class="input bg-white text-black-2 input-bordered w-full" value="{{ $data->nama }}" required>
            </div>
            <div class="form-control">
                <label for="alamat" class="text-black-2 dark:text-white mb-2">Alamat</label>
                <input type="text" name="alamat" id="alamat" class="input input-bordered text-black-2 bg-white w-full" value="{{ $data->alamat }}" required>
            </div>
            <div class="form-control">
                <label for="no_telepon" class="text-black-2 dark:text-white mb-2">No Telepon</label>
                <input type="text" name="no_telepon" id="no_telepon" class="input input-bordered text-black-2 bg-white w-full" value="{{ $data->no_telepon }}" required>
            </div>
            <div class="form-control">
                <label for="asal_instansi" class="text-black-2 dark:text-white mb-2">Asal Instansi</label>
                <input type="text" name="asal_instansi" id="asal_instansi" class="input input-bordered text-black-2 bg-white w-full" value="{{ $data->asal_instansi }}" required>
            </div>
